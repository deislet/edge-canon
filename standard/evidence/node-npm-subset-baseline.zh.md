...
| npm 与 CommonJS | [Node 24.20.0 package 文档](https://nodejs.org/download/release/v24.20.0/docs/api/packages.html#conditional-exports)定义 package.json、exports/imports、ESM/CommonJS，并明确条件对象的 key 顺序有语义：较早的匹配优先、`default` 应最后声明；Cloudflare bundler 处理 npm 与 polyfill | Deno 支持 package.json、bare import、CommonJS、npm cache/node_modules | npm 是构建能力，最终 artifact 统一为静态 ESM graph；Edge Canon 固定激活条件集合和 lock 编码，但不把 Node 的 package 声明顺序改成自定义 resolver 优先级 |
