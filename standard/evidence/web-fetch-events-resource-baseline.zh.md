...
标准只公布全部参考供应商最低档和三个一等后端均能兑现、并通过压力与故障 fixture 的下限。未公开的响应大小、wall time、`waitUntil`、子请求或并发保证继续阻断该维度，不能猜测为无限或零，也不能从 API 存在反推数值。更高供应商配额只作为运维信息，不进入应用可依赖语义。套餐变化只能更新未发布草案或产生新标准版本，不能改写已发布版本。
