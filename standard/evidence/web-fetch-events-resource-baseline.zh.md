...
| 请求 body | Cloudflare Free/Pro 100 MB，其他套餐更高 | 1 MB | 先取保守的 1,000,000 octet 正向下限，并只覆盖无内容编码、已知长度的请求；压缩、chunked、未知长度和超限处理继续待定 |
...
3. “MB”的供应商精确计量、压缩前后计量、chunked/未知长度 body 和超限提前拒绝位置。当前 Draft 用十进制 1,000,000 octet 作为两家都不低于的保守正向下限，不声称它就是任一供应商的原生边界。
...
标准只公布全部参考供应商最低档和三个一等后端均能兑现、并通过压力与故障 fixture 的下限。当前可审查候选是 10 ms HTTP CPU、50 次外部 `fetch`、6 个同时等待响应头的外连，以及 identity 编码、已知长度的 1,000,000 octet 请求 body 正向读取；公开证据已经足以让这些条款进入 Draft，但尚不足以冻结。未公开的响应大小、wall time、`waitUntil` 继续阻断该维度，不能猜测为无限或零，也不能从 API 存在反推数值。两家虽都公布 128 MB 执行环境上限，但计量包含项与并发归属不足以推导应用最低可用内存，所以也不据此新增保证。更高供应商配额只作为运维信息，不进入应用可依赖语义。套餐变化只能更新未发布草案或产生新标准版本，不能改写已发布版本。
