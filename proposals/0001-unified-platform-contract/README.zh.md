...
`routing-static-assets`、`canonical-build-artifact` 与 `web-platform-apis` 也已进入 Draft，并分别具有机器可读 schema、case、reference harness 与 provider-independent oracle；三者均已有 Linux、macOS 与 Windows reference evidence。`streams-websockets-background-work` 现已固定共同 identity Streams 与 best-effort `waitUntil` 子集，并明确把没有参考供应商运行时交集的 WebSocket 判为预部署拒绝；其三平台 reference evidence 尚待不可变 commit 后收集。另有 10 个 mandatory capability family 仍为 `pending`，所有 Draft 也都缺三个一等后端的真实完整证据，因此 proposal 不能被实现或市场材料解释为已经支持。
