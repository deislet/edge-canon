...
当前 `web-fetch-events` 已有[候选要求](../../standard/requirements/web-fetch-events.zh.md)、机器可读用例和覆盖首批五项用例的可执行 harness/oracle 草案；最低资源保证只有 CPU/子请求/外连并发三个 Draft 下限，其余空白、依赖能力族、后续用例及三个 provider adapter 尚未完成。其状态因此保持 `draft`，不能被实现或市场材料解释为已经支持。
